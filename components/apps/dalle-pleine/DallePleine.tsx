...
export default function DallePleine() {
  ...
  const openSettings = () => {
    setCartoucheDraft(cartouche);
    setShowSettings(true);
  };
  const resetCartouche = () => {
    setCartoucheDraft(DEFAULT_CARTOUCHE);
  };
  ...
  const loadProjects = (): Record<string, CalculationState> => {
    try {
      return JSON.parse(localStorage.getItem('ec2_projects') || '{}');
    } catch {
      return {};
    }
  };
  const saveProjectsToStorage = (projects: Record<string, CalculationState>) => {
    try {
      localStorage.setItem('ec2_projects', JSON.stringify(projects));
    } catch {}
  };
  ...
  const handleExportPDF = () => {
    ...
    printWindow.document.write(`
      ...
      <html lang="fr">
        <head>
          <meta charset="UTF-8">
          <title>${projectName} — Note de calcul</title>
          ${stylesHTML}
          <style>
    ...
    <div className={`min-h-screen ${isDarkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} p-4 md:p-8 font-sans`}>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-xl font-black text-slate-900 dark:text-slate-50 flex items-center gap-3">
                <Settings size={22} className="text-blue-500" /> Cartouche de la Note de Calcul
              </h2>
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Nom du bureau d'études</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Sous-titre / Slogan</label>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Sigle (logo)</label>
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Titre du document</label>
                <input type="text" value={cartoucheDraft.documentTitle} onChange={(e) => setCartoucheDraft({ ...cartoucheDraft, documentTitle: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Ingénieur / Rédacteur (visa)</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">E-mail de contact</label>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Téléphone</label>
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-100 dark:border-slate-700">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={saveCartouche} className="flex items-center gap-2 px-5 py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition-all shadow-lg shadow-blue-100">
            ...
            <div className="flex-1 min-w-[200px] flex items-end gap-2">
              <div className="flex-1">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800/60 border-none rounded-lg px-4 py-2 font-bold text-lg focus:ring-2 focus:ring-blue-100 transition-all"
                />
              </div>
              {savedProjects.length > 0 && (
                <div className="relative group">
                  <select 
                    ...
                    className="appearance-none bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-600 dark:text-slate-300 px-3 py-2 rounded-lg text-xs font-bold transition-colors cursor-pointer border-none outline-none"
                    value=""
                  >
        ...
        <AnimatePresence mode="wait">
            ...
            >
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <select 
                        value={state.appuiType} 
                        onChange={(e) => updateState({ appuiType: e.target.value as AppuiType })}
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        value={state.borderCond} 
                        onChange={(e) => updateState({ borderCond: e.target.value as BorderCondition })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <input 
                        ...
                        step="0.1"
                        value={state.Lx} 
                        onChange={(e) => updateState({ Lx: parseFloat(e.target.value) || 0 })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        ...
                        step="0.1"
                        value={state.Ly} 
                        onChange={(e) => updateState({ Ly: parseFloat(e.target.value) || 0 })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input 
                        ...
                        step="10"
                        value={state.h} 
                        onChange={(e) => updateState({ h: parseFloat(e.target.value) || 0 })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      />
                    </div>
                  </div>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        value={state.fyk} 
                        onChange={(e) => updateState({ fyk: parseInt(e.target.value) })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        value={state.phi} 
                        onChange={(e) => updateState({ phi: parseInt(e.target.value) })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <select 
                        ...
                        onChange={(e) => updateState({ exposure: e.target.value })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        value={state.structClass} 
                        onChange={(e) => updateState({ structClass: e.target.value })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        value={state.deltaCdev} 
                        onChange={(e) => updateState({ deltaCdev: parseInt(e.target.value) })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
              ...
              <div className="space-y-8">
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-sm">
                  ...
                  <div>
                    <SlabVisualization state={state} results={results} />
                  </div>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <select 
                        value={state.qkCategoryIndex} 
                        onChange={(e) => updateState({ qkCategoryIndex: parseInt(e.target.value) })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <select 
                        value={state.g1} 
                        onChange={(e) => updateState({ g1: parseFloat(e.target.value) })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                  ...
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <select 
                          value={state.checkPunching ? "true" : "false"} 
                          onChange={(e) => updateState({ checkPunching: e.target.value === "true" })}
                          className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <select 
                          value={state.columnPosition} 
                          onChange={(e) => updateState({ columnPosition: e.target.value as ColumnPosition })}
                          className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <input 
                          type="number" 
                          step="50"
                          value={state.columnSize} 
                          onChange={(e) => updateState({ columnSize: parseFloat(e.target.value) || 0 })}
                          className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                        />
                      </div>
                    </div>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-sm">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                       ...
                       <select 
                        value={state.diam_inf_x} 
                        onChange={(e) => updateState({ diam_inf_x: parseInt(e.target.value) })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl">
                        ...
                        <input
                          ...
                          step={5}
                          min={0}
                          value={espXDraft !== null ? espXDraft : String(Math.round(results.esp_x_final))}
                          onChange={(e) => {
                            const v = e.target.value;
                            setEspXDraft(v);
                            const parsed = parseFloat(v);
                            if (v !== '' && !isNaN(parsed) && parsed > 0) {
                              updateState({ esp_x_manual: parsed });
                            }
                          }}
                          onBlur={() => {
                            const parsed = parseFloat(espXDraft ?? '');
                            if (espXDraft === null || espXDraft === '' || isNaN(parsed) || parsed <= 0) {
                              updateState({ esp_x_manual: undefined });
                            }
                            setEspXDraft(null);
                          }}
                        ...
                        />
                        {state.esp_x_manual != null && (
                          <span className="text-[10px] text-slate-400 dark:text-slate-500">Calculé : {Math.round(results.esp_x_auto)} mm</span>
                        )}
                      </div>
                    ...
                    <div className="space-y-6">
                      <div className="space-y-2">
                         ...
                         {(state.appuiType === AppuiType.FourSides && !results.effectiveOneWay) ? (
                           <select 
                            value={state.diam_inf_y} 
                            onChange={(e) => updateState({ diam_inf_y: parseInt(e.target.value) })}
                            className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                          >
                            {DIAM_OPTIONS.map(d => <option key={d} value={d}>ϕ {d} mm</option>)}
                          </select>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl">
                          ...
                          <input
                            ...
                            step={5}
                            min={0}
                            value={espYDraft !== null ? espYDraft : String(Math.round(results.esp_y_final))}
                            onChange={(e) => {
                              const v = e.target.value;
                              setEspYDraft(v);
                              const parsed = parseFloat(v);
                              if (v !== '' && !isNaN(parsed) && parsed > 0) {
                                updateState({ esp_y_manual: parsed });
                              }
                            }}
                            onBlur={() => {
                              const parsed = parseFloat(espYDraft ?? '');
                              if (espYDraft === null || espYDraft === '' || isNaN(parsed) || parsed <= 0) {
                                updateState({ esp_y_manual: undefined });
                              }
                              setEspYDraft(null);
                            }}
                            className="w-full bg-transparent text-xl font-black text-slate-900 dark:text-slate-50 outline-none"
                          />
                          {state.esp_y_manual != null && (
                            <span className="text-[10px] text-slate-400 dark:text-slate-500">Calculé : {Math.round(results.esp_y_auto)} mm</span>
                          )}
                        </div>
                ...
                <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 shadow-sm">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                       <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Diamètre choisi</label>
                       <select 
                        value={state.diam_top ?? state.diam_inf_x} 
                        onChange={(e) => updateState({ diam_top: parseInt(e.target.value) })}
                        className="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl px-4 py-3 font-semibold focus:ring-2 focus:ring-blue-100 transition-all outline-none"
                      >
                        {DIAM_OPTIONS.map(d => <option key={d} value={d}>ϕ {d} mm</option>)}
                      </select>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl">
                        ...
                        <input
                          ...
                          step={5}
                          min={0}
                          value={espTopDraft !== null ? espTopDraft : String(Math.round(results.esp_top_final))}
                          onChange={(e) => {
                            const v = e.target.value;
                            setEspTopDraft(v);
                            const parsed = parseFloat(v);
                            if (v !== '' && !isNaN(parsed) && parsed > 0) {
                              updateState({ esp_top_manual: parsed });
                            }
                          }}
                          onBlur={() => {
                            const parsed = parseFloat(espTopDraft ?? '');
                            if (espTopDraft === null || espTopDraft === '' || isNaN(parsed) || parsed <= 0) {
                              updateState({ esp_top_manual: undefined });
                            }
                            setEspTopDraft(null);
                          }}
                          className="w-full bg-transparent text-xl font-black text-slate-900 dark:text-slate-50 outline-none"
                        />
                        {state.esp_top_manual != null && (
                          <span className="text-[10px] text-slate-400 dark:text-slate-500">Calculé : {Math.round(results.esp_top_auto)} mm</span>
                        )}
                      </div>
          ...
          {activeTab === 'help' && (
            <motion.div 
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 md:p-12 shadow-sm prose prose-slate max-w-none no-scrollbar overflow-y-auto max-h-[80vh]"
            >
              <div className="max-w-4xl mx-auto">
                <header className="border-b-2 border-blue-100 pb-10 mb-12 text-center">
                  <div className="inline-flex items-center justify-center w-20 h-20 bg-blue-50 dark:bg-blue-950/40 text-blue-600 rounded-2xl mb-6 shadow-sm border border-blue-100">
                    <HelpCircle size={40} />
                  </div>
                ...
                <section className="mb-16">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 leading-relaxed mb-6">
                    Cet onglet est le point d'entrée de votre projet. Une saisie rigoureuse garantit la validité du modèle de calcul.
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-slate-50 dark:bg-slate-800/60 p-6 rounded-2xl border border-slate-100 dark:border-slate-700">
                  ...
                  <div className="mt-6 p-6 rounded-2xl bg-amber-50/60 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                      Pour une dalle « 4 côtés », dès que Ly/Lx dépasse 2, le calcul bascule automatiquement en dalle-poutre unidirectionnelle (portée Lx uniquement).
                    </p>
                  </div>
                </section>
                ...
                <section className="mb-16">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 leading-relaxed mb-6">
                    Visualisez les paramètres dérivés et les sollicitations de calcul à l'État Limite Ultime (ELU).
                  </p>
                  <div className="space-y-4">
                ...
                <section className="mb-16">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 leading-relaxed mb-6">
                    Chaque contrôle affiche la valeur calculée face à sa limite normative, avec un code couleur vert (conforme) ou rouge (non conforme).
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {[
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-400 leading-relaxed">
                          Définit les diamètres et les espacements réels des nappes inférieures (Lx, Ly), <strong>librement modifiables</strong> (un bouton « Auto » permet de revenir au calcul automatique). L'algorithme calcule également la <strong>longueur d'ancrage (lbd)</strong> nécessaire.
                        </p>
                      </div>
                      <div className="flex-1">
                        <h4 className="font-bold text-blue-400 mb-4 tracking-wider uppercase text-xs">Section Nomenclature</h4>
                        <p className="text-sm text-slate-400 leading-relaxed">
                          Génère un tableau récapitulatif (BBS) avec le nombre de barres, les longueurs unitaires et le <strong>poids total</strong> par type d'acier.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <section className="mb-16">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 mb-6">
                    Le couronnement de votre étude. L'onglet « Note de Calcul » synthétise l'ensemble des hypothèses et des résultats dans un document professionnel structuré — table des matières, hypothèses matériaux, données géométriques, analyse des sollicitations, vérifications réglementaires, nomenclature BBS complète et bibliographie normative.
                  </p>
                  <div className="bg-blue-600 p-1 rounded-2xl mb-6">
                     <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-blue-200">
                        <p className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                          <FileText className="text-blue-600" size={18}/> Astuce :
                        </p>
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
                          Le bouton "Imprimer le Rapport" ouvre une fenêtre dédiée contenant uniquement la note de calcul, prête à être imprimée ou enregistrée en PDF.
                        </p>
                     </div>
                  </div>
                </section>
          ...
          {activeTab === 'report' && (
            <motion.div 
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-8 md:p-12 shadow-sm min-h-[600px] prose prose-slate max-w-none"
            >
      ...
      <div className="fixed bottom-6 right-6 z-40 flex items-center gap-1 bg-white/95 dark:bg-slate-800/95 backdrop-blur border border-slate-200 dark:border-slate-700 rounded-full shadow-xl p-1.5">
        <button
          ...
          title={activeTabIndex > 0 ? `Précédent : ${TAB_ORDER[activeTabIndex - 1].label}` : undefined}
          className="flex items-center gap-1.5 pl-3 pr-4 py-2.5 rounded-full font-semibold text-sm transition-all text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-30 disabled:pointer-events-none"
        >
        ...
        <button
          ...
          title={activeTabIndex < TAB_ORDER.length - 1 ? `Suivant : ${TAB_ORDER[activeTabIndex + 1].label}` : undefined}
          className="flex items-center gap-1.5 pl-4 pr-3 py-2.5 rounded-full font-semibold text-sm transition-all bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-30 disabled:pointer-events-none"
        >
