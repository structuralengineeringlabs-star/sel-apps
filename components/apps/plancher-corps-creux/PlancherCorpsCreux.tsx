...
export default function PlancherCorpsCreux() {
  ...
  const [geometrie, setGeometrie] = useState<Geometrie>({
    type_appui: 'continu',
    portee: 4.5,
    entraxe: 0.6,
    largeur_ame: 12,
    epaisseur_entrevous: 16,
    epaisseur_dalle: 4,
  });
  const [materiaux, setMateriaux] = useState<Materiaux>({ fck: 25, fyk: 500 });
  const [durabilite, setDurabilite] = useState<Durability>({
    exposition: 'XC1',
    classe_structurale: 'S4',
    dev_ecart: 0,
    enrobage_manuel: 0,
  });
  const [chargesDetails, setChargesDetails] = useState<ChargesDetails>({
    revetement_sol: 0.5,
    chape: 1.2,
    faux_plafond: 0.2,
    cloisons: 1.0,
    type_local: 'A',
    poids_entrevous_manuel: 0,
  });
  const [choixManuel, setChoixManuel] = useState<ChoixManuel>({
    nb_travee: 2,
    diametre_travee: 10,
    nb_appui: 1,
    diametre_appui: 8,
    nb_branches_etrier: 2,
    diametre_etrier: 6,
  });
  const [etriersParams, setEtriersParams] = useState<EtriersParams>({ cot_theta: 2.5 });
  const [infosProjet, setInfosProjet] = useState<InfosProjet>({
    nom: 'Plancher Corps Creux',
    reference: 'BC-2024-001',
    redacteur: 'J. DOE',
    date: new Date().toLocaleDateString('fr-FR'),
    typeCalcul: 'PLANCHER CORPS CREUX',
    auteur: 'Structural & Engineering Labs',
    copyright: 'Structural & Engineering Labs © 2026',
  });
  ...
  const resultats = useMemo(
    () => calculerResultats({ geometrie, materiaux, durabilite, chargesDetails, choixManuel, etriersParams }),
    [geometrie, materiaux, durabilite, chargesDetails, choixManuel, etriersParams]
  );

  const longueur_barre_travee = useMemo(
    () => (resultats ? geometrie.portee + 2 * (parseFloat(resultats.verifications.ancrage.lbd_mm) / 1000) : 0),
    [resultats, geometrie.portee]
  );

  const nombre_etriers = useMemo(() => {
    if (!resultats || resultats.ferraillage.etriers.erreur || resultats.ferraillage.etriers.s_cm <= 0) return 0;
    return Math.floor((geometrie.portee * 100) / resultats.ferraillage.etriers.s_cm) + 1;
  }, [resultats, geometrie.portee]);

  const longueur_etrier_unitaire = useMemo(
    () => longueurEtrier(geometrie, enrobage_nominal, choixManuel.diametre_etrier),
    [geometrie, enrobage_nominal, choixManuel.diametre_etrier]
  );

  const longueur_chapeau = useMemo(() => {
    if (!resultats) return 0;
    if (resultats.ferraillage.appui_calcule) return geometrie.portee * 0.5;
    const hauteur_m = (geometrie.epaisseur_dalle + geometrie.epaisseur_entrevous) / 100;
    const c_m = enrobage_nominal / 1000;
    const retombee_m = Math.max(hauteur_m - c_m - choixManuel.diametre_appui / 1000, 0.1);
    return geometrie.portee / 5 + retombee_m + 0.1;
  }, [resultats, geometrie, enrobage_nominal, choixManuel.diametre_appui]);

  const total_acier_kg = useMemo(() => {
    if (!resultats) return 0;
    const travee = resultats.ferraillage.choix_travee.nb * longueur_barre_travee * poidsLineaire(resultats.ferraillage.choix_travee.diametre);
    const appui = resultats.ferraillage.choix_appui.nb * longueur_chapeau * poidsLineaire(resultats.ferraillage.choix_appui.diametre);
    const etriers = nombre_etriers * longueur_etrier_unitaire * poidsLineaire(choixManuel.diametre_etrier);
    return travee + appui + etriers;
  }, [resultats, longueur_barre_travee, longueur_chapeau, nombre_etriers, longueur_etrier_unitaire, choixManuel.diametre_etrier]);

  const courbesM = useMemo(() => {
    if (!resultats) return '';
    const vals = resultats.courbes.M_vals;
    const maxM = Math.max(...vals.map(Math.abs), 0.1);
    const scale = 70 / maxM;
    return vals.map((M, i) => `${40 + (i / (vals.length - 1)) * 320},${90 - M * scale}`).join(' ');
  }, [resultats]);

  const courbesV = useMemo(() => {
    if (!resultats) return '';
    const vals = resultats.courbes.V_vals;
    const maxV = Math.max(...vals.map(Math.abs), 0.1);
    const scale = 70 / maxV;
    return vals.map((V, i) => `${40 + (i / (vals.length - 1)) * 320},${90 - V * scale}`).join(' ');
  }, [resultats]);

  return (
    <div className="w-full py-4 px-2 sm:px-4 font-sans text-slate-900">
      <div className="max-w-2xl mx-auto space-y-3 screen-only">
        ...
        <div className="p-1 border border-slate-200 bg-white rounded-md no-print flex gap-1">
          {ONGLETS.map((tab) => (
            <button
              key={tab.id}
        ...
        <div className="text-center no-print pt-2">
          <input
            className="text-[9px] font-bold text-gray-400 uppercase tracking-widest bg-transparent border-none text-center focus:ring-0 cursor-edit w-full"
...
function SyntheseCompacte({
  ...
  resultats,
  total_acier_kg,
  longueur_barre_travee,
  longueur_chapeau,
  nombre_etriers,
  longueur_etrier_unitaire,
  onVoirRapport,
}: {
  geometrie: Geometrie;
  setGeometrie: (g: Geometrie) => void;
  materiaux: Materiaux;
  setMateriaux: (m: Materiaux) => void;
  durabilite: Durability;
  setDurabilite: (d: Durability) => void;
  chargesDetails: ChargesDetails;
  setChargesDetails: (c: ChargesDetails) => void;
  choixManuel: ChoixManuel;
  setChoixManuel: (c: ChoixManuel) => void;
  etriersParams: EtriersParams;
  setEtriersParams: (e: EtriersParams) => void;
  enrobage_nominal: number;
  resultats: Resultats | null;
  total_acier_kg: number;
  longueur_barre_travee: number;
  longueur_chapeau: number;
  nombre_etriers: number;
  longueur_etrier_unitaire: number;
  onVoirRapport: () => void;
}) {
  const [avanceOuvert, setAvanceOuvert] = useState(false);

  return (
    <motion.div key="synth" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <GroupeCompact titre="Géométrie">
          <EntreeSelect
            label="Système"
            value={geometrie.type_appui}
            onChange={(v) => setGeometrie({ ...geometrie, type_appui: v as AppuiType })}
            options={[
              { value: 'simple', label: 'Isostatique' },
              { value: 'continu', label: 'Continue' },
            ]}
          />
        ...
        <GroupeCompact titre="Matériaux & Durabilité">
          <EntreeSelect
            label="Béton"
            value={materiaux.fck}
            onChange={(v) => setMateriaux({ ...materiaux, fck: +v })}
            options={[20, 25, 30, 35].map((v) => ({ value: v, label: classeBeton(v) }))}
          />
          <EntreeNombre label="Acier fyk (MPa)" value={materiaux.fyk} onChange={(v) => setMateriaux({ ...materiaux, fyk: v })} />
          <EntreeSelect
            label="Exposition"
            value={durabilite.exposition}
            onChange={(v) => setDurabilite({ ...durabilite, exposition: v as Durability['exposition'] })}
            options={['XC1', 'XC2', 'XC3', 'XC4', 'XD1', 'XS1'].map((v) => ({ value: v, label: v }))}
          />
          <EntreeSelect
            label="Classe struct."
            value={durabilite.classe_structurale}
            onChange={(v) => setDurabilite({ ...durabilite, classe_structurale: v as Durability['classe_structurale'] })}
            options={[
              { value: 'S4', label: 'S4 (standard)' },
              { value: 'S5', label: 'S5 (+10%)' },
              { value: 'S6', label: 'S6 (+20%)' },
            ]}
          />
          <LigneChamp label="Enrobage cnom">
            <span className="text-xs font-bold text-[#0078d4]">{enrobage_nominal} mm</span>
          </LigneChamp>
        </GroupeCompact>
        ...
        <GroupeCompact titre="Charges">
          <EntreeSelect
            label="Usage"
            value={chargesDetails.type_local}
            onChange={(v) => setChargesDetails({ ...chargesDetails, type_local: v as ChargesDetails['type_local'] })}
            options={[
              { value: 'A', label: 'A · Habitation' },
              { value: 'B', label: 'B · Bureaux' },
              { value: 'C1', label: 'C1 · Enseign.' },
              { value: 'C2', label: 'C2 · Réunions' },
              { value: 'C3', label: 'C3 · Dense' },
              { value: 'D1', label: 'D1 · Commerces' },
            ]}
          />
          {CHAMPS_CHARGES.map((f) => (
            <EntreeNombre key={f.key} label={`${f.label} (kN/m²)`} value={chargesDetails[f.key]} onChange={(v) => setChargesDetails({ ...chargesDetails, [f.key]: v })} step="0.1" />
          ))}
        </GroupeCompact>
        ...
        <GroupeCompact titre="Armatures">
          ...
          <EntreeSelect
            label="Ø travée (mm)"
            value={choixManuel.diametre_travee}
            onChange={(v) => setChoixManuel({ ...choixManuel, diametre_travee: +v })}
            options={[8, 10, 12, 14, 16].map((d) => ({ value: d, label: `HA${d}` }))}
          />
          <EntreeSelect
            label="Ø étrier (mm)"
            value={choixManuel.diametre_etrier}
            onChange={(v) => setChoixManuel({ ...choixManuel, diametre_etrier: +v })}
            options={[6, 8, 10].map((d) => ({ value: d, label: `HA${d}` }))}
          />
          <EntreeSelect
            label="Inclinaison cotθ"
            value={etriersParams.cot_theta}
            onChange={(v) => setEtriersParams({ cot_theta: +v })}
            options={[1.0, 1.25, 1.5, 1.75, 2.0, 2.25, 2.5].map((v) => ({ value: v, label: `${v}` }))}
          />
        </GroupeCompact>
      </div>
      ...
      <div className="win-card p-3">
        ...
        {avanceOuvert && (
          <div className="mt-2 pt-2 border-t border-slate-100">
            ...
            <EntreeSelect
              label="Nb barres chapeau"
              value={choixManuel.nb_appui}
              onChange={(v) => setChoixManuel({ ...choixManuel, nb_appui: +v })}
              options={[1, 2, 3].map((n) => ({ value: n, label: `${n}` }))}
            />
            <EntreeSelect
              label="Ø chapeau (mm)"
              value={choixManuel.diametre_appui}
              onChange={(v) => setChoixManuel({ ...choixManuel, diametre_appui: +v })}
              options={[8, 10, 12, 14, 16].map((d) => ({ value: d, label: `HA${d}` }))}
            />
            <EntreeSelect
              label="Brins étrier"
              value={choixManuel.nb_branches_etrier}
              onChange={(v) => setChoixManuel({ ...choixManuel, nb_branches_etrier: +v })}
      ...
      {!resultats ? (
        ...
        <>
            ...
            {[
              ...
              <div key={s.label} className="bg-slate-50 rounded-md p-2 text-center">
                ...
                <div className="text-sm font-bold text-slate-800">
                  {s.value} <span className="text-[9px] font-normal text-slate-400">{s.unite}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <GroupeCompact titre="Ferraillage retenu">
            <div className="flex items-center justify-between gap-2 py-1.5 border-b border-slate-100 flex-wrap">
              ...
              <div className="flex items-center gap-1">
                <input
                  type="number"
                  value={choixManuel.nb_travee}
                  onChange={(e) => setChoixManuel({ ...choixManuel, nb_travee: +e.target.value })}
                  className="win-input text-right"
                  style={{ width: '38px', height: '24px', padding: '2px 4px', fontSize: '11px' }}
                />
                <select
                  value={choixManuel.diametre_travee}
                  onChange={(e) => setChoixManuel({ ...choixManuel, diametre_travee: +e.target.value })}
                  className="win-input"
                  style={{ width: '62px', height: '24px', padding: '2px 4px', fontSize: '11px' }}
                >
                  {[8, 10, 12, 14, 16].map((d) => (
              ...
              <span className="text-[11px] text-slate-600">
                {formatAsCm2(resultats.ferraillage.As_travee)} → {resultats.ferraillage.choix_travee.as_fourni.toFixed(2)} cm²
              </span>
            ...
            <div className="flex items-center justify-between gap-2 py-1.5 border-b border-slate-100 flex-wrap">
              ...
              <div className="flex items-center gap-1">
                <select
                  value={choixManuel.nb_appui}
                  onChange={(e) => setChoixManuel({ ...choixManuel, nb_appui: +e.target.value })}
                  className="win-input text-right"
                  style={{ width: '38px', height: '24px', padding: '2px 4px', fontSize: '11px' }}
                >
                  {[1, 2, 3].map((n) => (
                ...
                <select
                  value={choixManuel.diametre_appui}
                  onChange={(e) => setChoixManuel({ ...choixManuel, diametre_appui: +e.target.value })}
                  className="win-input"
                  style={{ width: '62px', height: '24px', padding: '2px 4px', fontSize: '11px' }}
                >
                  {[8, 10, 12, 14, 16].map((d) => (
              ...
              <span className="text-[11px] text-slate-600">
                {formatAsCm2(resultats.ferraillage.As_appui)} → {resultats.ferraillage.choix_appui.as_fourni.toFixed(2)} cm²
              </span>
            ...
            <div className="flex items-center justify-between gap-2 py-1.5 flex-wrap">
              ...
              <div className="flex items-center gap-1">
                <select
                  value={choixManuel.diametre_etrier}
                  onChange={(e) => setChoixManuel({ ...choixManuel, diametre_etrier: +e.target.value })}
                  className="win-input"
                  style={{ width: '62px', height: '24px', padding: '2px 4px', fontSize: '11px' }}
                >
                  {[6, 8, 10].map((d) => (
              ...
              <span className="text-[11px] text-slate-600">
                {resultats.ferraillage.etriers.erreur ? resultats.ferraillage.etriers.statut : `espacement s = ${resultats.ferraillage.etriers.s_cm} cm`}
              </span>
            ...
            <table className="w-full text-[11px]">
              ...
              <tfoot>
                <tr className="border-t border-slate-200">
                  <td colSpan={3} className="pt-1 text-right text-[10px] text-slate-400 uppercase">
                    Total
                  </td>
          ...
          <p className="text-[9px] text-slate-400 italic px-1">
            ...
            {!resultats.verifications.ancrage.conforme_40phi && ' — le forfait usuel "40Ø" était insuffisant ici, lbd calculé utilisé'}. Forme chapeau :{' '}
            {formeChapeau(resultats.ferraillage.appui_calcule)}. Forme étrier : {formeEtrier(geometrie, enrobage_nominal).label}
          </p>
        </>
      )}
...
<button
  onClick={() => {
    ...
    if (!noteElement) {
      alert('Note de calcul introuvable');
      return;
    }
    ...
    printWindow.document.write(`
      ...
      <html lang="fr">
        <head>
          <meta charset="UTF-8">
          <title>Note de calcul — ${infosProjet.nom}</title>
          ${stylesHTML}
          <style>
            * { box-sizing: border-box; }
            ...
            .no-print, header, footer, .screen-only { display: none !important; }
            .note-de-calcul {
            ...
            .note-de-calcul h2, .note-de-calcul h3, .note-de-calcul h4 {
              page-break-after: avoid;
              break-after: avoid;
            }
            .note-de-calcul table, .note-de-calcul tr, .note-de-calcul svg {
...
>
  ...
  EXPORTER EN PDF
          ...
          <div className="note-de-calcul print:shadow-none print:p-0 font-serif">
            ...
            <div className="space-y-12">
              ...
              <div className="space-y-4">
                ...
                <table className="mt-0">
                  <thead>
                    <tr><th>Désignation</th><th>G (Permanent)</th><th>Q (Variable)</th></tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td>Poids propre plancher (G0), dont entrevous {resultats.details_charges.poids_entrevous_surf} kN/m²</td>
                      <td className="text-center">{resultats.details_charges.G0_surf}</td>
                      <td className="text-center">-</td>
                    </tr>
                    <tr>
                      <td>Charges permanentes additionnelles (G')</td>
                      <td className="text-center">{resultats.details_charges.G_prime_surf}</td>
                      <td className="text-center">-</td>
                    </tr>
                    <tr>
                      <td>Charges d'exploitation (Usage {chargesDetails.type_local})</td>
                      <td className="text-center">-</td>
                      <td className="text-center">{resultats.details_charges.Q_surf}</td>
                    </tr>
                    <tr className="bg-gray-100 font-bold">
                      <td>TOTAUX SURFACIQUES</td>
                      <td className="text-center">{(+resultats.details_charges.G0_surf + +resultats.details_charges.G_prime_surf).toFixed(2)}</td>
                      <td className="text-center">{resultats.details_charges.Q_surf}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                <table className="mt-0">
                  <thead>
                    <tr><th>Section</th><th>État</th><th>Moment Max (kN.m)</th><th>Tranchant (kN)</th></tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td><strong>Milieu de travée</strong></td>
                      <td>ELU</td>
                      <td className="text-center">{resultats.sollicitations.M_ed_travee}</td>
                      <td className="text-center">-</td>
                    </tr>
                    <tr>
                      <td></td>
                      <td>ELS</td>
                      <td className="text-center">{resultats.sollicitations.M_els_travee}</td>
                      <td className="text-center">-</td>
                    </tr>
                    <tr>
                      <td><strong>Appuis</strong></td>
                      <td>ELU</td>
                      <td className="text-center">{resultats.sollicitations.M_ed_appui}</td>
                      <td className="text-center">{resultats.sollicitations.V_ed}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                <div className="grid grid-cols-2 gap-6 mt-6 border-y border-gray-100 py-6">
                  <div className="space-y-2">
                    ...
                    <svg viewBox="0 0 400 180" width="100%" className="overflow-visible bg-gray-50/30 rounded border border-gray-100">
                      ...
                      <text x="200" y="20" textAnchor="middle" fontSize="11" fontWeight="black" fill="#334155">
                        MEd,max = {resultats.sollicitations.M_ed_travee} kN.m
                      </text>
                    </svg>
                  </div>
                  <div className="space-y-2">
              ...
              <div className="space-y-4">
                ...
                <p>
                  Le calcul est conduit en section en Té. Moment réduit µ ={' '}
                  {((+resultats.sollicitations.M_ed_travee / 1000) / (+resultats.ferraillage.fcd * geometrie.entraxe * Math.pow(+resultats.ferraillage.d_eff / 100, 2))).toFixed(3)}.
                </p>
                <table className="mt-0">
                  <tbody>
                ...
                {resultats.ferraillage.etriers.erreur && (
                  <p className="text-red-700 font-bold bg-red-50 border border-red-200 rounded-md p-3">⚠ {resultats.ferraillage.etriers.statut}</p>
                )}
                <table className="mt-0">
                  <tbody>
              ...
              <div className="space-y-4">
                ...
                <table className="mt-0">
                  <thead>
                    <tr><th>Élément</th><th>Contrainte σ (MPa)</th><th>Limite σlim (MPa)</th><th>Vérification</th></tr>
                  </thead>
                  <tbody>
                    <tr>
              ...
              <table className="mt-0">
                <thead>
                  <tr><th>Vérification</th><th>Critère</th><th>Taux de travail</th><th>Statut</th></tr>
                </thead>
                <tbody>
                  <tr>
                    <td>Ferraillage longitudinal (travée)</td>
                    <td>As,prov ≥ As,th</td>
                    <td className="text-center">{formatRatioPct(resultats.verifications.ferraillage.ratio)}</td>
                    <td className={`text-center font-bold ${resultats.verifications.ferraillage.conforme ? 'text-emerald-700' : 'text-red-700'}`}>
                      {resultats.verifications.ferraillage.conforme ? 'CONFORME' : 'NON CONFORME'}
                    </td>
                  </tr>
                  <tr>
                    <td>Flexion (résistance ultime)</td>
                    <td>MEd ≤ MRd</td>
                    <td className="text-center">{formatRatioPct(resultats.verifications.flexion.ratio)}</td>
                    <td className={`text-center font-bold ${resultats.verifications.flexion.conforme ? 'text-emerald-700' : 'text-red-700'}`}>
                      {resultats.verifications.flexion.conforme ? 'CONFORME' : 'NON CONFORME'}
                    </td>
                  </tr>
                  <tr>
                    <td>Effort tranchant (étriers + bielle)</td>
                    <td>VEd ≤ VRd</td>
                    <td className="text-center">{formatRatioPct(resultats.verifications.effort_tranchant.ratio)}</td>
                    <td className={`text-center font-bold ${!resultats.ferraillage.etriers.erreur && resultats.verifications.effort_tranchant.conforme ? 'text-emerald-700' : 'text-red-700'}`}>
                      {resultats.ferraillage.etriers.erreur ? 'ÉCRASEMENT BIELLE' : resultats.verifications.effort_tranchant.conforme ? 'CONFORME' : 'NON CONFORME'}
                    </td>
                  </tr>
                  <tr>
                    <td>Flèche (élancement L/d)</td>
                    <td>L/d ≤ (L/d)lim</td>
                    <td className="text-center">{resultats.verifications.fleche?.L_d_reel} / {resultats.verifications.fleche?.L_d_limite}</td>
                    <td className={`text-center font-bold ${resultats.verifications.fleche?.conforme ? 'text-emerald-700' : 'text-red-700'}`}>
                      {resultats.verifications.fleche?.conforme ? 'CONFORME' : 'NON CONFORME'}
                    </td>
                  </tr>
                  <tr>
                    <td>Contraintes ELS (béton + acier)</td>
                    <td>σc ≤ 0.6·fck ; σs ≤ 0.8·fyk</td>
                    <td className="text-center">{resultats.verifications.els?.sigma_c}/{resultats.verifications.els?.limite_c} — {resultats.verifications.els?.sigma_s}/{resultats.verifications.els?.limite_s} MPa</td>
                    <td className={`text-center font-bold ${resultats.verifications.els?.conforme ? 'text-emerald-700' : 'text-red-700'}`}>
                      {resultats.verifications.els?.conforme ? 'CONFORME' : 'NON CONFORME'}
                    </td>
                  </tr>
                  <tr>
                    <td>Pourcentage d'armature maximal</td>
                    <td>As ≤ As,max = 0.04·Ac (§9.2.1.1(3))</td>
                    <td className="text-center">{resultats.ferraillage.choix_travee.as_fourni.toFixed(2)} / {resultats.ferraillage.choix_appui.as_fourni.toFixed(2)} ≤ {resultats.verifications.pourcentage_armature.As_max_cm2} cm²</td>
                    <td className={`text-center font-bold ${resultats.verifications.pourcentage_armature.conforme ? 'text-emerald-700' : 'text-red-700'}`}>
                      {resultats.verifications.pourcentage_armature.conforme ? 'CONFORME' : 'NON CONFORME'}
                    </td>
                  </tr>
                  <tr>
                    <td>Espacement maximal des étriers</td>
                    <td>s ≤ min(0.75d ; 30 cm) (§9.2.2(6))</td>
                    <td className="text-center">{resultats.ferraillage.etriers.s_cm} ≤ {resultats.verifications.espacement_etriers.s_max_cm} cm</td>
                    <td className={`text-center font-bold ${resultats.verifications.espacement_etriers.conforme ? 'text-emerald-700' : 'text-red-700'}`}>
                      {resultats.verifications.espacement_etriers.conforme ? 'CONFORME' : 'NON CONFORME'}
                    </td>
                  </tr>
                  <tr>
                    <td>Longueur d'ancrage (travée)</td>
                    <td>lbd calculé (§8.4)</td>
                    <td className="text-center">{resultats.verifications.ancrage.lbd_mm} mm</td>
                    <td className="text-center font-bold text-emerald-700">APPLIQUÉ</td>
                  </tr>
                  <tr className={resultats.est_valide ? 'bg-emerald-50 font-bold' : 'bg-red-50 font-bold'}>
                    <td colSpan={3}>CONCLUSION GÉNÉRALE</td>
                    <td className={`text-center ${resultats.est_valide ? 'text-emerald-700' : 'text-red-700'}`}>{resultats.est_valide ? 'STRUCTURE CONFORME' : 'STRUCTURE NON CONFORME'}</td>
                  </tr>
                </tbody>
              </table>
              ...
              <table className="mt-0">
                <thead>
                  <tr><th>Rep.</th><th>Type / Façonnage</th><th>Forme</th><th>Diam. (mm)</th><th>Nb.</th><th>Long. (m)</th><th>Masse (kg/m)</th><th>Poids Total (kg)</th></tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="text-center font-bold">01</td>
                    <td>Aciers de travée</td>
                    <td className="text-xs">{formeTravee(resultats.verifications.ancrage.lbd_mm)}</td>
                    <td className="text-center">HA{resultats.ferraillage.choix_travee.diametre}</td>
                    <td className="text-center">{resultats.ferraillage.choix_travee.nb}</td>
                    <td className="text-center">{longueur_barre_travee.toFixed(2)}</td>
                    <td className="text-center">{poidsLineaire(resultats.ferraillage.choix_travee.diametre).toFixed(3)}</td>
                  ...
                  <tr>
                    <td className="text-center font-bold">02</td>
                    <td>{resultats.ferraillage.appui_calcule ? 'Aciers de chapeaux' : 'Aciers de construction (chapeau)'}</td>
                    <td className="text-xs">{formeChapeau(resultats.ferraillage.appui_calcule)}</td>
                    <td className="text-center">HA{resultats.ferraillage.choix_appui.diametre}</td>
                    <td className="text-center">{resultats.ferraillage.choix_appui.nb}</td>
                    <td className="text-center">{longueur_chapeau.toFixed(2)}</td>
                  ...
                  <tr>
                    <td className="text-center font-bold">03</td>
                    <td>Étriers / Cadres ({choixManuel.nb_branches_etrier} brins)</td>
                    ...
                    <td className="text-center">HA{choixManuel.diametre_etrier}</td>
                    <td className="text-center">{nombre_etriers}</td>
                    <td className="text-center">{longueur_etrier_unitaire.toFixed(2)}</td>
                    <td className="text-center">{poidsLineaire(choixManuel.diametre_etrier).toFixed(3)}</td>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div className="schema-box p-4">
                  ...
                  <table className="mt-2 text-xs">
                    <thead>
                      <tr><th>Diamètre</th><th>Poids Total (kg)</th></tr>
                    </thead>
                    <tbody>
                      {Array.from(
                        new Set(
                          [resultats.ferraillage.choix_travee.diametre, resultats.ferraillage.choix_appui.diametre, choixManuel.diametre_etrier].filter((v): v is number => Boolean(v))
                        )
                      ).map((d) => {
                        const p =
                          (resultats.ferraillage.choix_travee.diametre === d ? resultats.ferraillage.choix_travee.nb * longueur_barre_travee * poidsLineaire(d) : 0) +
                          (resultats.ferraillage.choix_appui.diametre === d ? resultats.ferraillage.choix_appui.nb * longueur_chapeau * poidsLineaire(d) : 0) +
                          (choixManuel.diametre_etrier === d ? nombre_etriers * longueur_etrier_unitaire * poidsLineaire(d) : 0);
                        return (
                          <tr key={d}>
                            <td><strong>HA {d}</strong></td>
